# Created by Ashish Patel at 2025/01/15 20:55
# leetgo: 1.4.13
...
Out of these, the subarray `[3, -2]` has a sum of 1, which is the smallest positive sum. Hence, the
answer is 1.
...
There is no subarray of length between `l` and `r` that has a sum greater than 0. So, the answer is -
1.
...
class Solution:
    def minimumSumSubarray(self, nums: List[int], l: int, r: int) -> int:
        ...
        for left in range(len(acc)-l):
            for right in range(left + l, min(left+r+ 1, len(acc))):
                temp = acc[right] - acc[left]
                if temp > 0:
...
if __name__ == "__main__":
    ...
    l = 2
    r = 3
    ans = Solution().minimumSumSubarray(nums, l, r)
